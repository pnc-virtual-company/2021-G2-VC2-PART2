      ...
      <v-dialog v-model="dialogDelete" max-width="450px">
        <v-card>
          <v-card-title class="red--text"
            >Are you sure you want to remove this user?</v-card-title
          >
          <hr />
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn depressed color="primary" @click="dialogDelete = false"
              >Cancel</v-btn
            >
      ...
      <v-card color="" green>
        <v-simple-table>
          <template v-slot:default>
            ...
            <tbody>
              <tr class="" v-for="(user, index) in userList" :key="index">
                <td>
                  <v-img
                    height="80"
                    width="80"
                ...
                <td>{{ user.role }}</td>
                <td>
                  <v-icon mediem class="mr-2" @click="editItem(user)"
                    >mdi-pencil</v-icon
                  >
                  <v-icon
                    mediem
